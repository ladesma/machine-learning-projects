# -*- coding: utf-8 -*-
"""
Created on Fri May 13 01:55:05 2022
...
def pso(n_particles, iterations, dimensions, inertia):
    ...
    k = np.random.rand(n_particles, 1)*(max_k - min_k) + min_k
    x = np.round(x)
    x = x.astype(int)
    y = np.round(y)
    y = y.astype(int)
    ...
    c = np.concatenate((x,y,z,k), axis=1)
    ...
    for i in range(iterations):
        ...
        for j in range(n_particles):
          ...
          if c[j][3] < 3:
            ...
            c[j][3] = 6
          parameterd=c[j][3].item()
          print("Parametro C ",parameterc," parametro gamma ",pgamma, " kernel ",kernel1, " degree", parameterd)
          if parameterd < 3 or parameterd > 7:
              accuracy=0.0
          elif parameterc == 1:
              accuracy=0.0
          else:
              rgs =svm.SVC(kernel=kernel1, C=parameterc, gamma=pgamma, degree=parameterd)
